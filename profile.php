...
if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty($_POST['action']) && $_POST['action'] == 'post_delete') {
  ...
  die;
} else if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty($_POST['action']) && $_POST['action'] == 'post_edit') {
  //post edit
  ...
  $image_added = false;
  if (
    !empty($_FILES['image']['name']) && $_FILES['image']['error'] == 0 &&
  ...
  <div style="margin: auto; max-width: 600px;">
    ...
    <?php if (!empty($_GET['action']) && $_GET['action'] == 'post_delete' && !empty($_GET['id'])) : ?>
      ...
      <?php if (mysqli_num_rows($result) > 0) : ?>
        ...
        <form method="post" enctype="multipart/form-data" style="margin:auto; padding:10px;">
          ...
          <div name="post"><?= $row['post'] ?></div><br>
          <input type="hidden" name="action" value="post_delete">
          <button class="my-btn">delete</button>
          <a href="profile.php">
            <button class="my-btn" type="button">Cancel</button>
          </a>
        </form>
      <?php endif; ?>
    <?php elseif (!empty($_GET['action']) && $_GET['action'] == 'post_edit' && !empty($_GET['id'])) : ?>
      ...
      <?php if (mysqli_num_rows($result) > 0) : ?>
        ...
        <form method="post" enctype="multipart/form-data" style="margin:auto; padding:10px;">
          ...
          <textarea name="post" id="" rows="8"><?= $row['post'] ?></textarea><br>
    ...
    <?php elseif (!empty($_GET['action']) && $_GET['action'] == 'edit') : ?>
      ...
      <form method="post" enctype="multipart/form-data" style="margin:auto; padding:10px;">
        ...
        <img src="<?php echo $_SESSION['info']['image'] ?>" style="width: 150px; height: 150px; object-fit: cover; margin: auto; display: block;">
        image :<input type="file" name="image"><br>
    ...
    <?php else : ?>
      ...
      <div style=" margin:auto; max-width: 600px; text-align: center;">
        ...
        <a href="profile.php?action=edit">
          <button class="my-btn">Edit profile</button>
        </a>
        <a href="profile.php?action=delete">
          <button class="my-btn">Delete profile</button>
        </a>
      </div>
      <br>
      <hr>
